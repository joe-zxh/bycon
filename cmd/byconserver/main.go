...
import (
	...
	"github.com/joe-zxh/bycon"
	"github.com/joe-zxh/bycon/client"
...
func main() {
	...
	replicaConfig.QuorumSize = (replicaConfig.ClusterSize+f)/2 + 1 // bycon: q>(n+f)/2

	srv := newBYCONServer(&conf, replicaConfig)
...
func newBYCONServer(conf *options, replicaConfig *config.ReplicaConfig) *byconServer {
	...
	srv.initPayloadData()
	srv.bycon = bycon.New(replicaConfig, conf.TLS, time.Minute, time.Duration(conf.ViewTimeout)*time.Millisecond)
	srv.gorumsSrv.RegisterClientServer(srv)
	return srv
}

func (srv *byconServer) initPayloadData() { // 初始化一些常用的负载的大小
	srv.pldatas[0] = []byte(``)
...
func (srv *byconServer) getPayloadData(size int32) []byte {
...
func (srv *byconServer) Start(address string) error {
	lis, err := net.Listen("tcp", address)
	if err != nil {
		return err
	}

	err = srv.bycon.Start() // 这里是hs的入口
	if err != nil {
		return err
	}
...
func (srv *byconServer) Stop() {
	srv.gorumsSrv.Stop()
	srv.cancel()
	srv.bycon.Close()
}

func (srv *byconServer) ExecCommand(_ context.Context, cmd *client.Command, out func(*client.Empty, error)) {
	...
	// if srv.bycon.IsLeader {
	if srv.bycon.IsLeader {
		// fmt.Printf("ID: %d, seq: %d\n", srv.bycon.ID, cmd.SequenceNumber)
		...
		srv.bycon.AddCommand(data.Command(b))
		srv.bycon.Propose(false)
	}
...
func (srv *byconServer) AskViewChange(_ context.Context, _ *client.Empty, out func(*client.Empty, error)) {

	if srv.bycon.IsLeader {
		srv.bycon.StartViewChange()
	}

	<-srv.bycon.ViewChangeChan

	// send response
	out(&client.Empty{}, nil)
}

func (srv *byconServer) RoundTrip(_ context.Context, _ *client.Empty, out func(*client.Empty, error)) {
	// send response
	out(&client.Empty{}, nil)
}

func (srv *byconServer) onExec() {
	for cmds := range srv.bycon.GetExec() {
